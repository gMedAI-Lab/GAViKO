...
class ExplicitVisualPrompting(nn.Module):
    ...
    def load_pretrain(self, backbone):
        ...
        def mean_kernel(patch_emb_weight):
            ...
            depth = self.conv_proj.proj.weight.shape[2]
        ...
        for key, value in jax_dict.items():
            if key == 'cls_token':
                ...
                new_key = key.replace('patch_embed.proj.weight', 'conv_proj.proj.weight')
                value = mean_kernel(value)
                add_item(new_key, value)
            elif 'patch_embed.proj.bias' in key:
                new_key = key.replace('patch_embed.proj.bias', 'conv_proj.proj.bias')
                add_item(new_key, value)
            elif key == 'pos_embed':
